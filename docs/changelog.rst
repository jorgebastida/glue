...
* This version introduce a new concept named ``Formats``. Every output file generated by glue now is created using a format. This change decouples glue's core (as far as possible) from how the output is created allowing glue developers to create different formats without messing glue's core. Fixin #64
...
* Glue now use ``argparse`` instead of ``optparse``.
...
* ``--margin`` now support more than only one margin, you can now use it with (i.e) ``--margin=10 20 10 20``. Fixes #101
